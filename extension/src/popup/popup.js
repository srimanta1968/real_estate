...
async function handleOpenSites() {
  ...
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span> Searching sites...';
  ...
  // Send to background worker — it opens tabs, waits, scrapes, and forwards to DealEval
  // Background worker persists even after popup closes
  chrome.runtime.sendMessage({
    type: 'START_SITE_SEARCH',
    urls: urlsToOpen.map(u => ({ url: u.url, hostname: u.hostname })),
  });

  btn.textContent = 'Sites opened! Scraping in background...';
  btn.style.background = '#ecfdf5';
  btn.style.color = '#065f46';
  ...
  // Re-enable after 3s (scraping continues in background even if popup closes)
  setTimeout(() => {
    btn.disabled = false;
    btn.textContent = 'Open Search on Selected Sites';
